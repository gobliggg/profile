...
func Start(options ...func(*Profile)) interface {
  ...
  switch prof.mode {
  case cpuMode:
    fn := filepath.Join(path, fmt.Sprintf("cpu-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case memMode:
    fn := filepath.Join(path, fmt.Sprintf("mem-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case mutexMode:
    fn := filepath.Join(path, fmt.Sprintf("mutex-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case blockMode:
    fn := filepath.Join(path, fmt.Sprintf("block-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case threadCreateMode:
    fn := filepath.Join(path, fmt.Sprintf("threadcreation-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case traceMode:
    fn := filepath.Join(path, fmt.Sprintf("trace-%s.out", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
  ...
  case goroutineMode:
    fn := filepath.Join(path, fmt.Sprintf("goroutine-%s.pprof", time.Now().Format("2006-01-02T15-04-05")))
    f, err := os.Create(fn)
    if err != nil {
